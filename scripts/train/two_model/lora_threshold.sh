...
mkdir -p checkpoints/two_model_lora
...
CUDA_VISIBLE_DEVICES=4,5,6,7 ${PYTHON} train_new.py \
  ...
  --mode threshold --reward-threshold 1.0 \
  --max-steps 100 \
  ...
  --run-name "GRPO_TwoModels_LoRA_Threshold_$(date +%Y%m%d_%H%M%S)" \
  > logs/two_model_lora_threshold.log 2>&1 &

PID=$!

echo "Two-model LoRA collaborative training started! PID=$PID"
echo "Check logs: tail -f logs/two_model_lora_threshold.log"

wait $PID
echo "Two-model LoRA collaborative training completed!"
