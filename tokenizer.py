...
class Tokenizer:
    ...
    def __init__(self, tokenizer_path: str):
        ...
        # Get bos_token if available
        self.bos_token = self.tokenizer_config.get("bos_token", "")
        # Handle models that don't have pad_token (like Llama)
        if "pad_token" in self.tokenizer_config:
            self.pad_token = self.tokenizer_config["pad_token"]
        else:
            # Use eos_token as pad_token for models that don't have pad_token
