...
def apply_rotary_pos_emb(q, k, cos, sin, unsqueeze_dim=2):
    ...
    Supports Group Query Attention where q and k may have different numbers of heads.
    """
    # Handle both 2D and 4D cos/sin tensors
    if cos.dim() == 4:
        # cos/sin shape: [batch, 1, seqlen, head_dim] -> squeeze to [batch, seqlen, head_dim]
        cos = cos.squeeze(1)  # [batch, seqlen, head_dim]
        sin = sin.squeeze(1)  # [batch, seqlen, head_dim]
    elif cos.dim() == 2:
        # cos/sin shape: [seqlen, head_dim] -> add batch dim
        cos = cos.unsqueeze(0)  # [1, seqlen, head_dim]
        sin = sin.unsqueeze(0)  # [1, seqlen, head_dim]
    
    # Now cos/sin should be [batch, seqlen, head_dim]
    # q shape: [bsz, seqlen, n_q_heads, head_dim]
    # k shape: [bsz, seqlen, n_kv_heads, head_dim]
    
    # Expand cos and sin for broadcasting with q and k
    cos_q = cos.unsqueeze(2).expand(-1, -1, q.size(2), -1)  # [batch, seqlen, n_q_heads, head_dim]
    sin_q = sin.unsqueeze(2).expand(-1, -1, q.size(2), -1)  # [batch, seqlen, n_q_heads, head_dim]
    cos_k = cos.unsqueeze(2).expand(-1, -1, k.size(2), -1)  # [batch, seqlen, n_kv_heads, head_dim]
    sin_k = sin.unsqueeze(2).expand(-1, -1, k.size(2), -1)  # [batch, seqlen, n_kv_heads, head_dim]
...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(self, config: LlamaConfig, device: torch.device):
        super().__init__()
        self.config = config
...
class Transformer(nn.Module):
    def __init__(self, params: LlamaConfig, device: torch.device):
        super().__init__()
        self.params = params
        ...
        with torch.device(device):
            self.rotary_emb = LlamaRotaryEmbedding(config=params, device=device)
    ...
    def from_pretrained(cls, ckpt_path, device: torch.device):
        ...
        args = LlamaConfig(
            ...
            bos_token_id=config.get("bos_token_id", 128000),
            eos_token_id=config.get("eos_token_id", 128009),
        ...
        weights = {k.replace("model.", "") if k.startswith("model.") else k: v for k, v in weights.items()}
        model.load_state_dict(weights, strict=True, assign=True)
        return model.to(device)
    
def main():
    model = Transformer.from_pretrained("Llama-3.2-3B-Instruct", device=torch.device("cuda"))
    print(model)
